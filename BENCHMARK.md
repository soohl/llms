...
Latest local agentic and web-research results for all four configured models,
as of 2026-08-14. The suites measure different capabilities, so their scores
are shown together for convenience but are not added into a single score.
...
Muse completed the most agentic tasks, while Muse and DeepSeek tied on
agentic points. DeepSeek, Gemma, and Muse tied on research points. All four
...
| Task | Difficulty | DeepSeek V4 Flash | Gemma 4 | Muse Glimmer | Qwen3.6 |
| --- | --- | ---: | ---: | ---: | ---: |
| Python repair | Easy | FAIL 4/6 | **PASS 6/6** | **PASS 6/6** | FAIL 4/6 |
| Data reconciliation | Medium | FAIL 10/12 | FAIL 10/12 | FAIL 8/12 | FAIL 10/12 |
| Dependency planner | Hard | **PASS 20/20** | FAIL 0/20 | **PASS 20/20** | FAIL 0/20 |
...
Gemma and Qwen's dependency-planner submissions did not expose the required
`parse_tasks` API, causing the hidden grader import to fail. Muse used
substantially more turns and generated tokens than the other models and was
the only RTX 4090 model to pass the hard task.
...
| Task | Difficulty | DeepSeek V4 Flash | Gemma 4 | Muse Glimmer | Qwen3.6 |
| --- | --- | ---: | ---: | ---: | ---: |
| Python standards | Easy | **PASS 8/8** | **PASS 8/8** | **PASS 8/8** | **PASS 8/8** |
| HTTP retry policy | Medium | **PASS 14/14** | **PASS 14/14** | **PASS 14/14** | **PASS 14/14** |
| XZ incident | Hard | FAIL 15/20 | FAIL 15/20 | FAIL 15/20 | FAIL 9/20 |
...
mitigation, plus the required mix of primary, maintainer, and vendor
citations. Qwen additionally missed build targeting and the impact dependency
path.
